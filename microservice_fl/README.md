# MicroServiceFL — 微服务故障定位智能体

> 面向微服务系统的实时故障定位智能体：从可观测性数据（调用链 / 指标 / 日志）自动将故障定位到 **服务 → 类 → 方法**，给出**根因分析**与**修复建议**。支持无人值守的实时自动诊断，也支持按需一次性定位。基于 [OpenHarness](https://github.com/HKUDS/OpenHarness) 智能体框架构建。
...
- **数据接入层（`datasource/`）**：`DataSource` 接口 + 两种实现——`DuckDBDataSource`（离线）、`SkyWalkingDataSource`（在线，调用链实时查 OAP，指标/日志读采集 CSV）。
- **异常检测层（`monitor/`）**：`StatDetector` 基于中位数 + MAD 的稳健 3σ 检测（资源指标附带绝对下限）；`SignatureDetector` 对日志做内容感知的**新错误签名**检测，捕捉"数量未超阈值但出现了新错误"的低频高危问题。
- **采集层（`collectors/`）**：psutil 采集各服务 CPU/内存、tail 服务日志（保留异常堆栈），写入带保留期自动裁剪的 CSV。
- **定位能力层（`tools/` + `/locate` 技能）**：`fl_*` 系列工具（扫服务、拓扑、接口异常、耗时分解、错误 span/日志、接口→类映射、反编译等），由 `/locate` 技能编排大模型按固定方法链推理。
